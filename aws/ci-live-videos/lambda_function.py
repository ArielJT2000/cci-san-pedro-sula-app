...
def _get_from_youtube(api_key, channel_id):
    ...
    try:
        with urllib.request.urlopen(live_url, timeout=HTTP_TIMEOUT_SEC) as live_response:
...
def _write_to_dynamodb(live_video_id):
    """Guarda en DynamoDB. notified solo True cuando hay videoId (hay transmisión)."""
    try:
        has_live = bool(live_video_id)
        table.put_item(
            Item={
                "status": "live",
...
def _send_fcm_notification(project_id, access_token, topic, title, body, data):
    """Envía notificación push FCM. Lanza urllib.error.HTTPError si FCM responde error."""
    ...
    try:
        with urllib.request.urlopen(req, timeout=HTTP_TIMEOUT_SEC) as resp:
            ...
            if raw:
                print(f"FCM response: {raw}")
    except urllib.error.HTTPError as e:
        err_body = e.read().decode() if e.fp else ""
        print(f"FCM HTTP {e.code}: {err_body}")
        raise
...
def lambda_handler(event, context):
    ...
    if _is_api_invocation(event):
        ...
        return {
            "statusCode": 200,
            "headers": headers,
            "body": json.dumps({"liveVideoId": live_id or ""}),
        }
        ...
        "body": json.dumps(
            {
                ...
                "message": (
                    "Test completado. Datos guardados en DynamoDB."
                    if live_video_id
                    else "No hay transmisión en vivo detectada."
                ),
            }
        ),
    }
